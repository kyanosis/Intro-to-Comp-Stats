---
title: "IntroComp_WeixuanChen-HW12"
author: "Weixuan Chen"
date: "April 15, 2023"
output:
...
```
So, for elastic net regression, when $\alpha=0.5$ will give the best value of $\lambda=1$ which is 1.434335
...
mse.e
```
Thus, we observe that MSE for Elastic net model is 4.79 way less than for Linear regression model with value 20.113. Hence, Elastic net works much better than Multiple Linear Regression.
...
lmout$coefficients
```
On comparing, the coefficients of the Elastic net model with the Multiple Linear Regression model we observe that, 3 coefficients of Elastic net which are disp, drat, gear have been shrunk to 0 by the model unlike Linear regression. Thus, Elastic net has a property of coefficient shrinkage and thereby feature reduction.

Thus, this means that these 3 variables do not affect the dependent variable y (mpg).
...
heart.data <- read.csv("/Users/primordial/Desktop/heart_tidy.csv", header = T, sep = ",")
x <- heart.data[,c(1:13)]
y <- heart.data$X0.2
...
Thus linear kernel in this case outperforms radial kernel with a cost of 1 and performance of 0.82 better than radial with 0.813333
...
```
Accuracy comes as 81.3% with cost taken as 1 for radial kernel from previous result.
...
Thus, we observe that the Radial kernel (0.82) gives slightly better result than the logistic regression model.
